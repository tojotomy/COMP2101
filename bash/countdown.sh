...
function error-message {
        local prog
        prog=`basename $0`
        echo "${prog}: ${1:-Unknown Error - that movie was good}" >&2
}
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        * )
            usage
            error-exit "$1 not recognized"
    esac
    shift
done

if [ ! $numberOfSleeps -gt 0 ]; then
    error-exit "$numberOfSleeps is not a valid count of sleeps"
fi

if [ ! $sleepTime -gt 0 ]; then
    error-exit "$sleepTime is not a valid time to sleep"
fi

sleepCount=$numberOfSleeps

while [ $sleepCount -gt 0 ]; do
    echo $((sleepCount * 100 / $numberOfSleeps))|dialog --gauge "Remain time" 7 60
    sleepCount=$((sleepCount - 1))
    sleep $sleepTime
done

echo "Wait counter expired, exit peacefully"
